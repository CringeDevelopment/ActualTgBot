...
async def WelcomeProcess(message: types.Message):
    await message.answer('Здравствуйте, введите название мероприятия', reply_markup=types.ReplyKeyboardRemove())
    await CreatingSteps.Title.set()


async def UploadTitle(message: types.Message, state: FSMContext):
    await state.update_data(title=message.text)
    await message.answer('Выберите тип мероприятия', reply_markup=EventTypeMenu)
...
async def ChooseType(message: types.Message, state: FSMContext):
    if message.text == 'Внутреннее' or message.text == 'Внешнее':
        ...
        await message.answer('Добавьте описание', reply_markup=types.ReplyKeyboardRemove())
        await CreatingSteps.next()
    else:
        await message.reply(f'{message.from_user.full_name}, возникла какая-то ошибка, повторите, пожалуйста, еще раз', reply_markup=EventTypeMenu)
        return


async def UploadDescription(message: types.Message, state: FSMContext):
    await state.update_data(description=message.text)
    await message.answer('Добавьте дату')
    await CreatingSteps.next()
...
async def UploadDate(message: types.Message, state: FSMContext):
    ...
    await message.answer('Добавьте ссылку на интернет-ресурс, если не нужно, то введите "нет"')
    await CreatingSteps.next()


async def ErrDate(message: types.Message, state: FSMContext):
    await message.reply(f'{message.from_user.full_name}, возникла какая-то ошибка, повторите, пожалуйста, еще раз')
    return


async def UploadWebSource(message: types.Message, state: FSMContext):
    await state.update_data(source=message.text)
    await message.answer('Добавьте фото, если не нужно, то введите "нет"')
    await CreatingSteps.next()
...
async def UploadPhoto(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    result = await sql_events.add_event(data['title'], data['type'], data['description'], data['date'], data['photo'],
                                        0, data['source'])
...
def register_CreatingEventHandler(dp: Dispatcher):
    ...
    dp.register_message_handler(UploadWebSource, lambda message: re.search(r'(https?://[\S]+)', message.text),
                                state=CreatingSteps.WebSource)
