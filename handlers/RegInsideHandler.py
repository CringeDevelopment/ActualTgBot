...
from keyboards.reply_keyboards import  UserMainMenu, RoleMenu
...
async def TypeProcess(message : types.Message, state : FSMContext):
	try:
		...
		return
	if state.update_data == "aspirant":
		await message.answer(f'На кого собеседуешься',reply_markup = ApplicantRoleMenu)
		aspirantState = message.caption
	else:
...
async def AddInsideProcess(message : types.Message, state : FSMContext):
	if message.caption is None:
		...
		res = await sql_users.add_user(message.from_user.id, message.caption, data['type'], message.photo[0].file_id)
		if res == 606:
